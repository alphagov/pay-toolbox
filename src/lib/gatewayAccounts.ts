...
export function aggregateServicesByGatewayAccountId(services: Service[]): Map<string, Service> {
  return services.reduce((accumulator: Map<string, Service>, service: Service) => {
...
export function toAccountSearchParams(filters: Filters): ListGatewayAccountsRequest {
  return {
    type:
      (filters.live === BooleanFilterOption.True && AccountType.Live) ||
      (filters.live === BooleanFilterOption.False && AccountType.Test) ||
      undefined,
