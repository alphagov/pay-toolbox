...
export default class Connector extends Client {
  ...
  charges = ((client: Connector) => ({
     ...
     */
    getGatewayComparisons(ids: string[]): Promise<GatewayStatusComparison[]> {
      return client._axios
     ...
     * @returns Array of gateway comparison objects
     */
    resolveDiscrepancy(id: string): Promise<GatewayStatusComparison[]> {
      return client._axios
  ...
  accounts = ((client: Connector) => ({
     ...
     */
    retrieve(id: string): Promise<GatewayAccount> {
      return client._axios
     ...
     * @returns Gateway account object
     */
    retrieveByExternalId(externalId: string): Promise<GatewayAccount> {
      return client._axios
     ...
     * @returns Gateway account object
     */
    retrieveByServiceExternalIdAndAccountType(serviceExternalId: string, accountType: string): Promise<GatewayAccount> {
      return client._axios
        .get(`/v1/api/service/${serviceExternalId}/account/${accountType}`)
        .then((response) => client._unpackResponseData<GatewayAccount>(response))
        .catch(
          handleEntityNotFound('Account by service external ID and account Type', `${serviceExternalId}:${accountType}`)
        )
    },
     ...
     */
    retrieveStripeCredentials(id: string): Promise<StripeCredentials> {
      return client._axios
     ...
     */
    retrieveStripeSetup(id: string): Promise<StripeSetup> {
      return client._axios
     ...
     */
    listCardTypes(id: string): Promise<ListCardTypesResponse> {
      return client._axios
     ...
     */
    list(filters: ListGatewayAccountsRequest = {}): Promise<ListGatewayAccountsResponse> {
     ...
     */
    async retrieveForService(filters: ListGatewayAccountsRequest = {}): Promise<GatewayAccount> {
      const params = _.omitBy(filters, _.isEmpty)
      ...
      if (accounts.length > 1) {
        throw new Error(
          `Multiple accounts for service ${filters.serviceIds} ${filters.type}, this is a legacy configuration`
        )
      }
     ...
     */
    create(params: CreateGatewayAccountRequest): Promise<CreateGatewayAccountResponse> {
      return client._axios
     ...
     */
    update(id: string, params: UpdateGatewayAccountRequest): Promise<void> {
      ...
      return client._axios.patch(`/v1/api/accounts/${id}`, payload).then(() => {
        return
      })
    ...
    updateStripeSetup(id: string, params: UpdateStripeSetupRequest): Promise<void> {
      ...
      return client._axios.patch(`/v1/api/accounts/${id}/stripe-setup`, payload)
    },

    addGatewayAccountCredentials(
      id: string,
      params: AddGatewayAccountCredentialsRequest
    ): Promise<GatewayAccountCredentials> {
      return client._axios
  ...
  cardTypes = ((client: Connector) => ({
     ...
     */
    list(): Promise<ListCardTypesResponse> {
      return client._axios
  ...
  eventEmitter = ((client: Connector) => ({
    emitByIdRange(startId: number, maxId: number, recordType: string, retryDelayInSeconds: number): Promise<void> {
      ...
      return client._axios.post('/v1/tasks/historical-event-emitter', null, { params }).then(() => {
        return
      })
    },
    ...
    emitByDate(startDate: string, endDate: string, retryDelayInSeconds: number): Promise<void> {
      ...
      return client._axios.post('/v1/tasks/historical-event-emitter-by-date', null, { params }).then(() => {
        return
      })
    },
  }))(this)
  ...
  parityChecker = ((client: Connector) => ({
    runParityCheck(
      ...
      return client._axios.post('/v1/tasks/parity-checker', null, { params }).then(() => {
        return
      })
    },
  }))(this)
  ...
  fixAsyncFailedStripeRefund = ((client: Connector) => ({
    fixStripeRefund(
      ...
      chargeId: string,
      refundId: string,
      params: AddGitHubAndZendeskCredential
    ): Promise<Refund> {
      return client._axios
