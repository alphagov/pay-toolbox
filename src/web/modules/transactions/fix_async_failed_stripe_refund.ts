...
export async function fixAsyncFailedStripeRefund(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const githubUserId = req.user!.username!

    if (!parentTransactionId) {
      throw new Error(`tx ${transaction.transaction_id} has no parent, likely not a refund`)
    }

    const parentTransaction = await Ledger.transactions.retrieve(parentTransactionId)

    await Connector.fixAsyncFailedStripeRefund.fixStripeRefund(gatewayAccountId, parentTransactionId, refundId, {
