...
export class Dashboard extends React.Component<DashboardProps, DashboardState> {
          ...
          if (this.state.connection.attempts > MAX_FAILED_CONNECTION_ATTEMPTS) {
            ...
            if (now - this.state.lastConnectionTick > this.state.connection.attempts * systemTickInterval) {
              log(
                now,
                windowNow,
                `Attempting to connect, time since last attempt ${now - this.state.lastConnectionTick}ms`
              )
              this.setState({
                connection: {
                  ...this.state.connection,
              ...
              Promise.resolve()
                ...
                .then((result) => {
                  log(now, windowNow, 'Connected: fetched aggregate sync', {
                    limit: moment(syncBufferTimestamp).utc().format(),
                  })
                  this.setState({
                    connection: {
                      ...this.state.connection,
        ...
        case ConnectionStatus.CONNECTED:
          ...
          if (this.state.sync?.pending) {
            if (windowNow >= this.state.sync.timestampTick) {
              ...
              const stagedQueued = this.state.queuedEvents.filter(
                (event) => event.timestamp && event.timestamp >= windowNow
              )
          ...
          if (
            ...
            this.state.connection.lastAggregateSyncTimestamp &&
            now - this.state.connection.lastAggregateSyncTimestamp > aggregateSyncFrequency
          ) {
            ...
            this.fetchAggregateSync(moment(syncBufferTimestamp).utc())
              .then((result) => {
                log(now, windowNow, 'Got the data for a scheduled aggregate sync', {
                  limit: moment(syncBufferTimestamp).utc().format(),
                })
                this.setState({
                  connection: {
            ...
            (this.state.connection.lastFetchedEventsTimestamp &&
              now - this.state.connection.lastFetchedEventsTimestamp > this.props.tickInterval * 1000)
          ) {
            this.setState({
              connection: {
                ...this.state.connection,
            ...
            const to = moment(windowNow)
              .add(this.props.tickInterval * 3, 'seconds')
              .utc()
            ...
            if (differenceDrift < this.props.tickInterval * 1000) {
              log(
                now,
                windowNow,
                `Detected too much drift between the window and current time window size(${windowSize}ms) drift(${differenceDrift}ms)`
              )
              this.setState({
                connection: {
                  ...this.state.connection,
            ...
            log(
              now,
              windowNow,
              `Fetch event window ${from.format(format)} ${to.format(format)} window size(${windowSize}ms) drift(${differenceDrift}ms)`
            )
            fetchEventTicker(from, to)
              .then((result) => {
                this.setState({
                  queuedEvents: [...this.state.queuedEvents, ...result.events],
                  connection: {
                    ...this.state.connection,
                    isFetching: false,
  ...
  async fetchAggregateSync(timestampFrom: moment.Moment): Promise<PendingAggregateSync> {
    ...
    return {
      ...
      transactionVolumesByHour: jsonToChartData(
        volumesByHour.data,
        this.state.date,
        volumesByHour.compareData,
        this.state.compareGraphs,
        this.state.compareDate
      ),
    }
  ...
  processQueuedEvents(cursor: number) {
    ...
    if (staging.length || ignored.length) {
      this.setState({
        ...
        activeEvents: [...staging.reverse(), ...this.state.activeEvents].slice(0, MAX_ACTIVE_TICKER_EVENTS),
  ...
  render() {
    const headerStatusColourMap: Partial<Record<ConnectionStatus, string>> = {
      ...
      <div>
        <div className="govuk-grid-row govuk-body govuk-!-margin-bottom-5">
          <div className="govuk-grid-column-full">
            <header>
              <div
                className="event-card govuk-header__container"
                style={{
                  color: 'white',
                  ...
                  borderBottomColor: headerStatusColourMap[this.state.connection.status],
                }}
              >
                <div className="govuk-header__logo" style={{}}>
                  <span className="govuk-header__link govuk-header__link--homepage">
                    <span className="govuk-header__logotype">
                      ...
                      <span className="govuk-header__logotype-text" style={{ marginLeft: '5px' }}>
                        GOV.UK
                      </span>
                    </span>
                ...
                <div className="govuk-header__content" style={{ paddingTop: '2px' }}>
                  <a href="#" className="govuk-header__link govuk-header__service-name">
                    {this.state.date.format('D MMMM YYYY')}
                  </a>
                </div>
              </div>
            </header>
        ...
        <div className="govuk-grid-row govuk-body govuk-!-margin-bottom-4">
          <div className="govuk-grid-column-full">
            <div className="event-card" style={{ padding: '15px', paddingLeft: '20px', paddingRight: '20px' }}>
              ...
              <p className="govuk-body-l" style={{ marginBottom: '20px' }}>
                <a href="https://www.payments.service.gov.uk" className="govuk-link govuk-link--no-visited-state">
                  GOV.UK Pay
                </a>{' '}
                is a free service, available to public sector organisations to take online payments.
              </p>
            </div>
          </div>
        </div>
        <div className="govuk-grid-row govuk-body govuk-!-margin-bottom-4">
          ...
          <div
            style={{ maxHeight: this.state.statsHeight, overflowY: 'hidden', minHeight: 5 }}
            className="govuk-grid-column-one-half"
          >
            <EventListPanel
              events={this.state.activeEvents}
              sync={this.state.sync}
              numberOfAggregateSyncs={this.state.numberOfAggregateSyncs}
              fetchedServices={this.state.fetchedServices}
              isFetching={this.state.connection.isFetching}
          ...
          <div className="govuk-grid-column-full govuk-body">
            <details className="govuk-details">
              <summary className="govuk-details__summary">
                <span className="govuk-details__summary-text">Display options</span>
              </summary>
              <div className="govuk-details__text">
                <div className="govuk-form-group">
                  <fieldset className="govuk-fieldset">
                    <legend className="govuk-fieldset__legend govuk-fieldset__legend--m">
                      <h1 className="govuk-fieldset__heading">Live payment events feed</h1>
                    </legend>
                    <div className="govuk-radios govuk-radios--small" data-module="govuk-radios">
                      <div className="govuk-radios__item">
                        <input
                          ...
                          onChange={this.setDisplayOptionEventTicker.bind(this)}
                        ></input>
                        <label className="govuk-label govuk-radios__label">Only show payment succeeded events</label>
                      ...
                      <div className="govuk-radios__item">
                        <input
                          ...
                          onChange={this.setDisplayOptionEventTicker.bind(this)}
                        ></input>
                        <label className="govuk-label govuk-radios__label">Show all events</label>
                        <div id="sign-in-item-hint-two" className="govuk-hint govuk-radios__hint">
                          Show cards for all payments events including when payments are created, when card details are
                          provided and when a payment has been successfully processed.
                        </div>
                      </div>
                    </div>
                  </fieldset>
...
function updateStagingGraphNode(transactionVolumesByHour: Serie[], event: Event, pointIndex: number) {
  ...
  const point = transactionVolumesByHour[pointIndex].data[index] || {
    x: `${moment(event.timestamp).format('YYYY-MM-DDTHH')}:00:00.000000Z`,
    y: 0,
  }
